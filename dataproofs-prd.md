...
**`kid` placement rule (v1.3.0, issue #25).** RFC 7515 imposes none: §4.1.4 calls `kid` a *hint*, and §6 states that such parameters "need not be integrity protected" when the only information used in the trust decision is a key, "since changing them in a way that causes a different key to be used will cause the validation to fail". Both placements are therefore conformant, and the choice belongs to the application profile — so `JwsSigner` carries a `JwsKidPlacement` (`Auto` | `Protected` | `Unprotected`). Under the `Auto` default, a JSON-serialized JWS declaring the DIDComm signed media type (`application/didcomm-signed+json`, or the bare `didcomm-signed+json` — DIDComm v2.1 §Message Formats permits omitting the `application/` prefix) emits `protected = {typ, alg}` with the `kid` in the per-signature unprotected `header`, matching every example in DIDComm v2.1 Appendix C.2 and both SICPA reference implementations; every other media type keeps the `kid` protected. Compact serialization always keeps it protected because RFC 7515 §7.1 gives it no unprotected header, and `BuildCompactAsync` rejects an explicit `Unprotected` rather than silently signing a `kid` the caller asked to leave unsigned. Two invariants hold across every placement: the `kid` is written to exactly one header, never both (RFC 7515 §7.2.1 disjointness, issue #17), and `JwsParser` accepts either placement while reporting `JwsParseResult.SignerKid` only after the signature verifies (issue #10).

**Verifier-side obligation.** §6's exemption is conditional — `kid` need not be integrity protected only when "the only information used in the trust decision is a key". An unprotected `kid` is therefore sound as a key *hint* but not as a signer *identity*: any identifier resolving to the same key material verifies equally well, and a DID document routinely lists one key under several verification-method ids, so an intermediary can relabel the `kid` between them without breaking the signature. `JwsParseResult.SignerKidIsProtected` reports whether the verified `kid` was covered by the signature; it is `false` both for the unprotected placement and when no `kid` was carried, so a verifier that derives a proof purpose, verification relationship, or authorization scope from the `kid` MUST require it and thereby fail closed.

**Serialization-form rule for the DIDComm signed media type (v1.3.0, issue #25).** A single signer normally produces the Flattened form (RFC 7515 §7.2.2), which is what the RFC 7520 cookbook `json_flat` byte-compare pins; two or more produce the General form. The DIDComm signed media type is the one exception: it always produces General, matching Appendix C.2, because didcomm-python 0.3.2 validates the **raw** envelope dict (`core/sign.py` `unpack_sign` → `core/validation.py` `validate_jws`) and rejects anything lacking a `signatures` array before its JOSE library normalizes the serialization. DIDComm v2.1 §Message Signing permits either form and requires recipients to process both, so this is an interop accommodation, not a conformance requirement — hence its scope is exactly the media type that needs it.

**FR-14 — JWE.** Compact and General JSON serialization, multi-recipient. Every JWE algorithm is **backed by a NetCrypto primitive; none is rolled locally.** Verified against NetCrypto's actual surface (`crypto-dotnet` `src/NetCrypto`, audited 2026-06-10): content encryption — `A256GCM` (`AesGcmCipher`), `A256CBC-HS512` (`AesCbcHmacCipher`), `XC20P` (`XChaCha20Poly1305Cipher`); key management — `A256KW` (`AesKeyWrap`), and the ECDH key-agreement variants assembled in `Jose` over NetCrypto's raw-ECDH-plus-KDF primitives (`ICryptoProvider.DeriveSharedSecret` returns the raw shared secret *Z* for X25519/P-256/P-384, and `ConcatKdf` derives the wrapping key — JOSE ECDH-ES/1PU use the NIST SP 800-56A Concat KDF (RFC 7518 §4.6), **not** HKDF, even though NetCrypto also exposes `Hkdf` for other uses). NetCrypto deliberately exposes *primitives*, not named JOSE algorithms, so `ECDH-ES+A256KW` and `ECDH-1PU+A256KW` (the latter pinned at `draft-madden-jose-ecdh-1pu-04`, required by the future didcomm adoption) are composed here — ECDH-1PU as Z = Ze‖Zs from two `DeriveSharedSecret` calls — with the APU/APV/Concat-KDF assembly ported from `didcomm-dotnet`, which already implements exactly this. (NetCrypto's ECDH covers X25519/P-256/P-384/P-521 — issue #61 *added* P-521, so curve availability is not a constraint; v1 JWE uses X25519/P-256/P-384, the curves the didcomm adoption needs, with P-521 left out of v1 scope by the FR-13 curve decision rather than by any gap.) The AC-3 checkpoint guards the standing invariant that no `Jose` algorithm bypasses NetCrypto. **(v1.1.0)** The ECDH key-agreement step also exposes an additive async `IEcdhKey` seam (`Crv` + `DeriveAsync` → raw *Z*), so a non-exporting `IKeyStore`-held key can drive JWE ECDH-ES/1PU decryption and authcrypt sending without surrendering its scalar — the JWE counterpart of the AC-8 signing invariant; the in-process `RawEcdhKey` reproduces the conformance vectors byte-for-byte and the existing sync API is unchanged. **(v1.1.0)** `JweParser.Parse` decrypt is constant-work with respect to recipient-key possession: a JWE addressing a recipient the holder cannot decrypt runs the same ECDH/key-unwrap against a per-process decoy key on the envelope's `epk` curve and fails with a single uniform exception, so neither response time nor the exception distinguishes which recipient private keys the holder possesses (issue #12; the resolver's own `FindPresent`/`TryGet` timing is the caller's responsibility).
