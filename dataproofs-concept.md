...
2. JOSE outputs cross-verify with the interop oracles for overlapping algorithms (a JWS/JWE produced here verifies in `jose-jwt` and vice versa, for algorithms both support). The **EdDSA** path is covered by RFC 8037's worked Appendix-A vectors. **ES256K** conforms to its RFC 8812 JOSE registration — but RFC 8812 is a registration document and ships no worked JWS vector, and `jose-jwt`'s closed algorithm enum excludes ES256K so there is no oracle either; ES256K coverage is therefore a generated vector set whose signatures are cross-verified at generation time against an independent secp256k1 implementation and frozen as regression vectors (PRD AC-3). SD-JWT and KB-JWT round-trip against RFC 9901 examples and `Owf.Sd.Jwt` cross-checks.
