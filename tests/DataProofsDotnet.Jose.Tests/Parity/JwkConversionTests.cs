...
/// OKP keys import raw bytes, so <see cref="JwkConversion.ToJsonWebKey"/> — the single chokepoint
...
public sealed class JwkConversionTests
{
    ...
    [InlineData(33)]
    [InlineData(64)]
    public void ToJsonWebKey_rejects_okp_key_whose_x_is_not_32_bytes(int length)
    {
        var jwk = new Jwk { Kty = "OKP", Crv = "Ed25519", X = Base64Url.Encode(new byte[length]), Kid = "did:example:a#k" };

        Action act = () => JwkConversion.ToJsonWebKey(jwk);

        act.Should().Throw<MalformedJoseException>().WithMessage("*32 bytes*");
    }

    [Fact]
    public void ToJsonWebKey_accepts_okp_key_with_32_byte_x()
    {
