...
using NetCrypto;
using Xunit;
using NetCryptoConcatKdf = NetCrypto.ConcatKdf;
using NetCryptoProvider = NetCrypto.DefaultCryptoProvider;

namespace DataProofsDotnet.Jose.Tests.Crypto.Kdf;

/// <summary>
/// Tests for <see cref="Ecdh1PuKdf"/>. ECDH-1PU's underlying primitives (raw ECDH + Concat KDF)
/// are tested upstream in NetCrypto against published vectors. The tests here pin the
/// 1PU-specific composition: Z = Ze ‖ Zs (ephemeral first, then static), and the AEAD
...
/// <see cref="Ecdh1PuKdf"/> entirely — we hand-assemble Z and SuppPubInfo and call NetCrypto's
/// public Concat KDF directly. If the two paths agree, the composition wiring is correct.
/// </remarks>
public sealed class Ecdh1PuKdfTests
{
    private readonly NetCrypto.ICryptoProvider _netCrypto = new NetCryptoProvider();
    ...
    public void Composition_matches_independent_reference_path()
    {
        ...
        var ze = _netCrypto.DeriveSharedSecret(KeyType.X25519, EphemeralPrivate, BobPublic);
        var zs = _netCrypto.DeriveSharedSecret(KeyType.X25519, AlicePrivate, BobPublic);
        ...
        var expected = NetCryptoConcatKdf.DeriveKey(
            sharedSecret: z,
