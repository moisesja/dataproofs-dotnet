...
public sealed class AuthcryptRoundTripTests
{
    ...
    public void Authcrypt_refuses_a256gcm_per_fr_enc_09()
    {
        ...
        Action act = () => JweBuilder.BuildEcdh1PuA256Kw(
    ...
    public void Unpack_without_sender_lookup_throws_for_authcrypt()
    {
        ...
        Action act = () => JweParser.Parse(packed, secrets, senderKeys: null, _crypto);
        act.Should().Throw<JoseCryptoException>().WithMessage("*sender*");
    }
    ...
    public void Unpack_rejects_authcrypt_whose_enc_is_not_cbc_hmac_per_fr_enc_09()
    {
        ...
        Action act = () => JweParser.Parse(tampered, secrets, senderLookup, _crypto);
        act.Should().Throw<JoseCryptoException>().WithMessage("*A256CBC-HS512*");
    }

    [Fact]
    public void Unpack_rejects_unsupported_content_encryption()
    {
        // An 'enc' outside the supported set must be rejected cleanly (JoseCryptoException), not
    ...
    public void Unpack_rejects_authcrypt_with_apu_not_matching_skid()
    {
        ...
        Action act = () => JweParser.Parse(tampered, secrets, senderLookup, _crypto);
        act.Should().Throw<JoseCryptoException>().WithMessage("*apu*does not match*");
    }
    ...
    public void Unpack_rejects_jwe_with_critical_extension()
    {
        ...
        Action act = () => JweParser.Parse(tampered, secrets, senderLookup, _crypto);
        act.Should().Throw<MalformedJoseException>().WithMessage("*crit*");
    }

    [Fact]
    public void Unpack_rejects_jwe_missing_required_member()
    {
        // A missing required member (here 'iv') must surface as MalformedJoseException, not a raw
        ...
        act.Should().Throw<MalformedJoseException>().WithMessage("*iv*");
    }
